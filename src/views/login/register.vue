<template>
    <div class='login-box' >
        <div class='box' v-show='registerDialog'>
            <div class='login-title' >{{$t('message.register.title')}}</div>
            <el-form ref="registerForm" :model="registerForm" label-position="top"  :validate-on-rule-change="false" :rules="registerRules" >
                ...
                <el-form-item prop='agree'  >
                    <div class='login-agree' >
                        <img src="~@/assets/image/no-check.png" alt=""  class='check-icon' v-if='!registerForm.agree' @click="checkAgree(true)">
                        <img src="~@/assets/image/check.png" alt=""  class='check-icon' v-else @click="checkAgree(false)">
                        <div class='login-text'>{{$t("message.register.agree")}}<a :href="user_agreement_url" target="_blank" rel="noopener noreferrer" class='agree-text' >{{$t("message.register.privacy")}}</a> {{$t("message.register.and")}} <a :href="privacy_policy_url" target="_blank"  class='agree-text' rel="noopener noreferrer">{{$t("message.register.useagree")}}. </a></div>
                    </div>
                </el-form-item>
                <el-form-item >
                    <el-button  :loading="loading" class="login-submit"  :disabled="comDiabled"  style="width: 100%" type="primary" @click.native.prevent="sendCode"  >
                        <span v-if="!loading">{{$t("message.register.verifyEmail")}}</span>
                        <span v-else>{{$t("message.register.verifyEmail")}}</span>
                    </el-button>
                </el-form-item>
            </el-form>
            <div class='account' >{{$t("message.register.hasAccount")}}? <span class='register-account' @click="logBtn">{{$t("message.register.login")}}</span> </div>
        </div>
        <div class='box'  v-show='codeDialog'>
            <div class='login-titles' > <img src="~@/assets/image/left-icon.png" alt="" class='left-icon' @click="back(2)"  >{{$t("message.register.verify")}}</div>
            <div class='tips'>{{$t("message.register.verifyTip")}} {{registerForm.username}}</div>
            <el-form ref="codeForm" :model="codeForm" label-position="top"   :rules="codeRules" >
         ...
         <div class='box' v-show='passwordDialog' >
            <div class='login-title' > <img src="~@/assets/image/left-icon.png" alt="" class='left-icon' @click="back(3)"> Set Password</div>
            <el-form ref="passwordForm" :model="passwordForm" label-position="top"  :validate-on-rule-change="false" :rules="passwordRules" >
               <el-form-item  prop='password' class='form-item-special' :label='$t("message.register.password")' >
...
import {sendEmailCode,verifymailcode,register,sendEmailTest,userExit} from '@/api/userApi'
...
export default {
    ...
    computed:{
        comDiabled(){
            ...
            if(upattern.test(this.registerForm.username)&&this.registerForm.agree){
                return false
            }else{
                return true
            }
    ...
    methods:{
        ...
        PasswordSubmit(){
            this.$refs.passwordForm.validate((valid) => {
                if(valid){
                    register({email:this.registerForm.username,password:this.$md5(this.passwordForm.new_password),code:this.codeForm.code}).then(response => {
                        this.loading=false
